...
let inFileTypesSARC = ["sarc", "aras", "baatarc", "lyarc"]
...
function decompressFromSARC (fileBuf) {
    ...
    let sarc = fileBuf.buf(0x00, 0x14)
        ...
        let sarc_byteOrder = sarc.int(0x06, IntSize.U16, {endian: Endian.BIG})
        ...
        let sarc_headerLength = sarc.int(0x04, IntSize.U16, {endian: numMode})
            FileBuf.expectVal(sarc_headerLength, 0x14, "SARC header states incorrect size")
        let sarc_version = sarc.int(0x10, IntSize.U16, {endian: numMode})
            FileBuf.expectVal(sarc_version, 0x100, "SARC header states incorrect version")
        let sarc_fileSize = sarc.int(0x08, IntSize.U32, {endian: numMode})
        let sarc_dataOffset = sarc.int(0x0C, IntSize.U32, {endian: numMode})
        let sarc_unused = sarc.int(0x12, IntSize.U16, {endian: numMode})
    let sfat = fileBuf.buf(0x14, 0x0C)
        ...
        let sfat_headerLength = sfat.int(0x04, IntSize.U16, {endian: numMode})
            FileBuf.expectVal(sfat_headerLength, 0x0C, "SFAT header states incorrect size")
        let sfat_hashKey = sfat.int(0x08, IntSize.U32, {endian: numMode})
            FileBuf.expectVal(sfat_hashKey, 0x65, "SFAT header states incorrect hash key")
        let sfat_nodeCount = sfat.int(0x06, IntSize.U16, {endian: numMode})
    let sfatNodes = fileBuf.buf(0x20, sfat_nodeCount * 0x10)
        ...
        for (let i = 0; i < sfat_nodeCount; i++) {
            ...
            sfatNodesList[i] = {
                fileNameHash: node.int(0x00, IntSize.U32, {endian: numMode}),
                fileAttributes: node.int(0x04, IntSize.U32, {endian: numMode}),
                fileDataStart: node.int(0x08, IntSize.U32, {endian: numMode}),
                fileDataEnd: node.int(0x0C, IntSize.U32, {endian: numMode}),
            }
        }
    ...
    let sfnt = fileBuf.buf(sfatNodesEnd, sfatNodesEnd + 0x08)
        ...
        let sfnt_headerLength = sfnt.int(0x04, IntSize.U16, {endian: numMode})
            FileBuf.expectVal(sfnt_headerLength, 0x08, "SFNT header states incorrect size")
        let sfnt_unused = sfnt.int(0x06, IntSize.U16, {endian: numMode})
