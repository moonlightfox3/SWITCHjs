...
function decompressFromRARC (fileBuf) {
    ...
    let header = fileBuf.buf(0x00, 0x20)
        ...
        let header_fileLength = header.int(0x04, IntSize.U32, {endian: numMode})
        let header_headerLength = header.int(0x08, IntSize.U32, {endian: numMode})
            FileBuf.expectVal(header_headerLength, 0x20, "RARC header states incorrect size")
        let header_fileDataOffset = header.int(0x0C, IntSize.U32, {endian: numMode})
        let header_fileDataLength = header.int(0x10, IntSize.U32, {endian: numMode})
        let header_fileDataLength2Unknown = header.int(0x14, IntSize.U32, {endian: numMode})
        let header_unused = header.int(0x18, IntSize.U64, {endian: numMode})
    let fileData = fileBuf.buf(header_fileDataOffset + 0x20, header_fileDataLength)
    let info = fileBuf.buf(0x20, 0x20)
        let info_nodeCount = info.int(0x00, IntSize.U32, {endian: numMode})
        let info_firstNodeOffset = info.int(0x04, IntSize.U32, {endian: numMode})
        let info_directoryCount = info.int(0x08, IntSize.U32, {endian: numMode})
        let info_firstDirectoryOffset = info.int(0x0C, IntSize.U32, {endian: numMode})
        let info_stringTableLength = info.int(0x10, IntSize.U32, {endian: numMode})
        let info_stringTableOffset = info.int(0x14, IntSize.U32, {endian: numMode})
        let info_fileDirectoryCount = info.int(0x18, IntSize.U16, {endian: numMode})
        let info_unused = info.int(0x1A, 0x06, {endian: numMode})
...
function rarc_getNode (fileBuf, numMode, index, structure) {
    let node = rarc_nodeTable[index]
        let id = node.str(0x00, 0x04, {endian: numMode})
        let stringOffset = node.int(0x04, IntSize.U32, {endian: numMode})
            let string = rarc_stringTable.str(stringOffset, rarc_stringTable.data.byteLength - stringOffset)
            string = string.split("\x00")[0]
        let stringHash = node.int(0x08, IntSize.U16, {endian: numMode})
        let directoryCount = node.int(0x0A, IntSize.U16, {endian: numMode})
        let firstDirectoryIndex = node.int(0x0C, IntSize.U32, {endian: numMode})
...
function rarc_getDirectory (fileBuf, numMode, offset) {
    let directory = fileBuf.buf(offset, 0x14)
        let index = directory.int(0x00, IntSize.U16, {endian: numMode})
            let isFolder = index == 0xFFFF
        let stringHash = directory.int(0x02, IntSize.U16, {endian: numMode})
        let stringOffset = directory.int(0x04, IntSize.U16, {endian: numMode})
            let string = rarc_stringTable.str(stringOffset, rarc_stringTable.data.byteLength - stringOffset)
            string = string.split("\x00")[0]
        let typeUnknown = directory.int(0x06, IntSize.U16, {endian: numMode})
            let isFolder2Unknown = null
            if (typeUnknown == 0x200) isFolder2Unknown = true
            else if (typeUnknown == 0x1100) isFolder2Unknown = false
        let offsetOrIndex = directory.int(0x08, IntSize.U32, {endian: numMode})
        let fileDataLength = directory.int(0x0C, IntSize.U32, {endian: numMode})
        let unused = directory.int(0x10, IntSize.U32, {endian: numMode})
